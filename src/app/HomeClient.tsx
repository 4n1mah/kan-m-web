...
// Presentación del home. La carga de datos (settings + destacados) y el ISR
// viven en src/app/page.tsx (server component); acá solo renderizamos con el
// idioma activo. El HTML del servidor sale en español (default) para SEO.

type HomeSettings = { catalogEnabled: boolean; quotesEnabled: boolean };
type FeaturedProducts = React.ComponentProps<typeof RotatingFeatured>["products"];
    ...
    <>
      ...
      <HeroCarousel images={IMAGES.heroCarousel} priorityFirst>
        <div className="max-w-7xl mx-auto px-6 py-14 md:py-20">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              {settings.quotesEnabled ? (
                ...
                <a
                  ...
                  className="btn-shine inline-flex items-center gap-2 px-6 py-3 rounded-full text-white font-semibold shadow-soft bg-gradient-rose"
                >
                  {t.home.ctaWriteUs} <ArrowRight size={18} />
                </a>
              )}
          ...
          <div className="float-y hidden sm:inline-flex mt-7 items-center gap-3 bg-white/15 backdrop-blur-sm rounded-2xl px-5 py-4 border border-white/30">
            ...
            <div>
              <div className="font-display text-lg leading-none text-white">{t.home.statEvents}</div>
              <div className="text-xs text-white/70 mt-1">{t.home.statEventsSub}</div>
            </div>
          </div>
        </div>
      </HeroCarousel>
      ...
      <section className="relative max-w-7xl mx-auto px-6 py-14 md:py-20 reveal">
        <div className="flex items-center justify-between gap-4 mb-8">
          <div>
            <p className="font-script text-xl md:text-2xl text-rose">{t.home.featuredKicker}</p>
            <h2 className="font-display text-2xl md:text-3xl mt-1">{t.home.featuredTitle}</h2>
          </div>
          <Link
            href="/catalogo"
          ...
          >
            {t.home.viewAll} <ArrowRight size={15} className="group-hover:translate-x-0.5 transition-transform" />
          </Link>
        </div>
      ...
      <section className="bg-secondary/40 py-14 md:py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-9 reveal">
            <p className="font-script text-2xl text-rose">{t.home.testimonialsKicker}</p>
            <h2 className="font-display text-2xl md:text-3xl mt-1">{t.home.testimonialsTitle}</h2>
            <a href="https://www.google.com/search?q=Kan+M+Reposteria+y+Catering" target="_blank" rel="noopener noreferrer"
              ...
              <span className="font-medium">{t.home.googleReviews}</span>
              <span className="text-muted-foreground/60">{t.home.seeAllReviews}</span>
            </a>
          </div>
            ...
            {[
              ...
              <a key={i} href="https://www.google.com/maps/search/Kan+M+Reposteria+y+Catering+Santo+Domingo" target="_blank" rel="noopener noreferrer"
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-medium text-muted-foreground">{t.home.reviewBadge}</span>
                </div>
                <div className="font-display text-4xl leading-none -mb-1" style={{ color: "#f07097" }}>&quot;</div>
                <p className="text-foreground/80 leading-relaxed mt-1 text-sm">{rev.q}</p>
        ...
        <HeroCarousel images={IMAGES.heroCarousel} autoplayMs={5000}>
          <div className="max-w-4xl mx-auto px-6 py-16 md:py-24 text-center">
            ...
            <div className="glass-strong rounded-3xl px-8 py-12 inline-block w-full">
              <p className="font-script text-2xl text-rose">{t.home.finalKicker}</p>
              <h2 className="font-display text-2xl md:text-4xl mt-2">
                {t.home.finalTitlePre} <span className="font-script text-gradient-animated">{t.home.finalTitleScript}</span>
              </h2>
              ...
              {settings.quotesEnabled ? (
                ...
                >
                  {t.home.ctaQuote} <ArrowRight size={20} />
                </Link>
              ) : (
                <a
                  href={waLink(WA_MESSAGES.general)}
                  ...
                  className="btn-shine mt-6 inline-flex items-center gap-2 px-6 py-3 rounded-full text-white text-base shadow-soft bg-gradient-rose"
                >
                  {t.home.finalCtaWhatsapp} <ArrowRight size={20} />
                </a>
              )}
            </div>
          </div>
