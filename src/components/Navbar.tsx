    ...
    >
      <div className="max-w-7xl mx-auto px-5 h-22 flex items-center justify-between gap-4" style={{ height: "5rem" }}>
        ...
        <nav className="hidden lg:flex items-center gap-1">
          ...
          {!isEmpanadoteca && visibleLinks.map((l) => {
            ...
            return (
              <Link
                ...
                className={`nav-link px-2 xl:px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all duration-150 ${
                  active
          ...
          {isEmpanadoteca ? (
            ...
            <a
              ...
              className="empanadoteca-link ml-1 flex items-center gap-1.5 px-3 xl:px-4 py-2 rounded-full bg-white/15 text-white text-sm font-semibold whitespace-nowrap"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="hidden lg:flex items-center gap-2 shrink-0">
          ...
          {/* Entre lg y xl no cabe junto a los links; el WhatsAppFab flotante lo cubre */}
          <a
            href={waLink(WA_MESSAGES.general)}
            target="_blank"
            rel="noopener noreferrer"
            className="hidden xl:flex items-center gap-1.5 px-4 py-2.5 rounded-full bg-white/20 border border-white/35 text-white text-sm font-medium hover:bg-white/30 transition-all backdrop-blur-sm"
          >
          ...
          {settings.quotesEnabled && !isEmpanadoteca && (
            <Link
              href="/cotizar"
              className="btn-shine flex items-center gap-1.5 px-5 py-2.5 rounded-full bg-white text-[#f07097] text-sm font-semibold whitespace-nowrap hover:bg-white/90 transition-all shadow-sm"
            >
              {t.nav.quote}
            </Link>
          )}
